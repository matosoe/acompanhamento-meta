...
public final class ExportFragment extends Fragment {
    ...
    @Override public void onViewCreated(@NonNull View view, @Nullable Bundle saved) {
        ...
        model.state().observe(getViewLifecycleOwner(), state -> { current=state; configureCategory(view, state.categories); adapter.submit(state.entries); summary.setText(getString(R.string.export_summary, state.entries.size())); error.setText(state.error); filters.setText(filterText()); });
    }
    ...
    private void saveToUri(@Nullable Uri uri) { if(uri==null) return; try(OutputStream out=requireContext().getContentResolver().openOutputStream(uri,"w")) { if(out==null) throw new java.io.IOException("Destino indisponível"); out.write(bytes()); Toast.makeText(requireContext(),getString(R.string.export_saved,current.entries.size()),Toast.LENGTH_LONG).show(); } catch(Exception error) { Toast.makeText(requireContext(),R.string.export_failed,Toast.LENGTH_LONG).show(); } }
