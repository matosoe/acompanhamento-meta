...
final class ExportEntryAdapter extends RecyclerView.Adapter<ExportEntryAdapter.Holder> {
    ...
    void submit(List<ExportEntryItem> values) { items.clear(); items.addAll(values); notifyDataSetChanged(); }
    ...
    @Override public void onBindViewHolder(@NonNull Holder holder, int position) {
        ...
        holder.time.setText(Instant.ofEpochMilli(item.entry.startEpochMillis).atZone(zone).format(DATE_TIME) + " – " + Instant.ofEpochMilli(item.entry.endEpochMillis).atZone(zone).format(DATE_TIME));
